...
else{ig.xhr('backend/',data,this.receivedData.bind(this));}},load:function(){if(window.Cocoon||window.Ejecta){}
else{ig.xhr('backend/',{load:true},this.receivedData.bind(this));}},receivedData:function(response){this.data=response.games;this.maxScore=0;for(var i=0;i<this.data.length;i++){var d=this.data[i];d.created=new Date(d.created*1000);this.maxScore=Math.max(this.maxScore,d.score);}
...
if(ig.input.mouse.y<104&&ig.input.pressed('click')){if(window.ejecta){
...
return elements;},splitIntoFragments:function(element,fragments){var parent=element.parentNode;var text=element.textContent;var sentences=text.replace(this._reSplitSentences,'$1%%ZT%%').split('%%ZT%%');var spans=[];for(var i=0;i<sentences.length;i++){var span=document.createElement('span');span.textContent=sentences[i];spans.push(span);fragments.push({text:sentences[i],element:span});}
...
else{this.player.miss();this.multiplier=1;this.streak=0;this.misses++;}}},setGame:function(){this.reset();this.gameTransitionTimer=new ig.Timer(2);var sx=ig.system.width/2-6,sy=ig.system.height-this.keyboard.height*this.keyboard.drawScale-30;this.player=this.spawnEntity(EntityPlayer,sx,sy);this.mode=ZType.MODE.GAME;this.nextWave();ig.music.next();this.spawnSound.play();this.emps=3;},setGameOver:function(){if(this.score>this.personalBest){this.isPersonalBest=true;this.personalBest=this.score;localStorage.setItem('highscore',this.personalBest);}
...
ig.System.drawMode=ig.System.DRAW.SUBPIXEL;ig.Sound.use=[ig.Sound.FORMAT.OGG,ig.Sound.FORMAT.MP3];if(window.Ejecta||window.Cocoon){ig.main('#canvas',ZType,60,width,height,1,ig.SilentLoader);}
